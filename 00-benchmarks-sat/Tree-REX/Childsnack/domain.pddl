...
(:method serve
		:parameters	(?c - child)
		:expansion	(
						(tag t1 (make_sandwich_no_gluten ?s ?b ?cont))
						(tag t2 (put_on_tray ?s ?t))
						(tag t3 (move_tray ?t kitchen ?p2))
						(tag t4 (serve_sandwich_no_gluten ?s ?c ?t ?p2))
						(tag t5 (move_tray ?t ?p2 kitchen))
			    	)
		:constraints
					(and 
			  			(before (and 
			  						(allergic_gluten ?c) (notexist ?s) (waiting ?c ?p2) (no_gluten_bread ?b) (no_gluten_content ?cont)
			  					) t1)
			     	)
)
...
(:method serve
		:parameters	(?c - child)
		:expansion	(
						(tag t1 (make_sandwich ?s ?b ?cont))
						(tag t2 (put_on_tray ?s ?t))
						(tag t3 (move_tray ?t kitchen ?p2))
						(tag t4 (serve_sandwich ?s ?c ?t ?p2))
						(tag t5 (move_tray ?t ?p2 kitchen))
			    	)
		:constraints
					(and 
			  			(before (and 
			  						 (not_allergic_gluten ?c) (notexist ?s) (waiting ?c ?p2) (not(no_gluten_bread ?b)) (not(no_gluten_content ?cont))
			  					) t1)
			     	)
)
